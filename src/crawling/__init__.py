...
def crawlSearchPage(nextPage, db):
    ...
    while True:
        ...
        with open(os.environ['DIR-WORKING'] + "/{}.html".format(hexDigestOfNextPage), 'w') as outfile:
...
def scrapeOverSearchPages():
    searchPages = list()
    file = open(os.environ['DIR-CONFIGS'] + '/searchpages.txt', 'r')
    for line in file:
        if line.strip() and not line.startswith("#"):
            searchPages.append(line.replace('\n', ''))

    # sort results by time to allow efficient scraping: already scraped items are ignored
    searchPages = ["{}&sortOption.sortBy=creationTime&sortOption.sortOrder=DESCENDING".format(updatedUrl) for updatedUrl
                   in searchPages]
    ...
    foundItems = crawlSearchPages(searchPages, db)
